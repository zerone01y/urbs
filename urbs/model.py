...
def create_model(data, dt=1, timesteps=None, objective="cost", dual=True):
    ...
    m.dt = pyomo.Param(initialize=dt, doc="Time step duration (in hours), default: 1")

    # import objective function information
    m.obj = pyomo.Param(
        initialize=objective, doc='Specification of minimized quantity, default: "cost"'
    )
    ...
    m.t = pyomo.Set(initialize=m.timesteps, ordered=True, doc="Set of timesteps")
    ...
    m.stf = pyomo.Set(
        initialize=indexlist, doc="Set of modeled support timeframes (e.g. years)"
    )
