...
def pyomo_model_prep(data):
    ...
    def invcost_factor(dep_prd, interest):
        ...
        return (1 + interest) ** dep_prd * interest / ((1 + interest) ** dep_prd - 1)

    m.process_dict = {}
    m.process_dict["invcost-factor"] = {
        p: invcost_factor(data["pro_depreciation"][p], data["pro_wacc"][p])
        for p in data["pro_wacc"].keys()
    }
...
def create_model(data, dt=1, objective="cost", dual=True):
    ...
    m.dt = pyomo.Param(
        initialize=data["dt"], doc="Time step duration (in hours), default: 1"
    )
    ...
    m.t = pyomo.Set(initialize=data["timesteps"], ordered=True, doc="Set of timesteps")
    ...
    # site
    m.sit = pyomo.Set(initialize=set(data["sites"]), doc="Set of sites")

    # commodity (e.g. solar, wind, coal...)
    m.com = pyomo.Set(initialize=set(data["commodities"]), doc="Set of commodities")

    # commodity type (i.e. SupIm, Demand, Stock, Env)
    m.com_type = pyomo.Set(
        initialize=set(data["com_type"]), doc="Set of commodity types"
    )

    # process (e.g. Wind turbine, Gas plant, Photovoltaics...)
    m.pro = pyomo.Set(
        initialize=set(data["process"]), doc="Set of conversion processes"
    )

    # cost_type
    m.cost_type = pyomo.Set(initialize=data["cost_type"], doc="Set of cost types")
    ...
    m.com_stock = pyomo.Set(
        within=m.com,
        initialize=set(
            com for stf, sit, com, com_type in m.com_tuples if com_type == "Stock"
        ),
    ...
    m.com_supim = pyomo.Set(
        within=m.com,
        initialize=set(
            com for stf, sit, com, com_type in m.com_tuples if com_type == "SupIm"
        ),
    ...
    m.com_demand = pyomo.Set(
        within=m.com,
        initialize=set(
            com for stf, sit, com, com_type in m.com_tuples if com_type == "Demand"
        ),
    ...
    m.com_env = pyomo.Set(
        within=m.com,
        initialize=set(
            com for stf, sit, com, com_type in m.com_tuples if com_type == "Env"
        ),
    ...
    m.res_vertex = pyomo.Constraint(
        m.tm, m.com_tuples, rule=res_vertex_rule, doc="process + source == demand"
    )
...
def commodity_balance(m, tm, stf, sit, com):
    ...
    balance = sum(
        ...
        if site == sit
        and stframe == stf
        and (stframe, process, com) in m._data["ratio_in"].keys()
    ) - sum(
        ...
        if site == sit
        and stframe == stf
        and (stframe, process, com) in m._data["ratio_out"].keys()
    )
    return balance
...
def res_stock_total_rule(m, stf, sit, com, com_type):
    if com not in m.com_stock:
        ...
        return total_consumption <= m._data["com_max"][(stf, sit, com, com_type)]
...
def res_env_total_rule(m, stf, sit, com, com_type):
    if com not in m.com_env:
        ...
        return env_output_sum <= m._data["com_max"][(stf, sit, com, com_type)]
...
def def_process_capacity_rule(m, stf, sit, pro):
    if (sit, pro, stf) in m.pro_const_cap_dict:
        ...
        cap_pro = m.cap_pro_new[stf, sit, pro] + m._data["pro_instcap"][(stf, sit, pro)]
    return cap_pro
...
def def_costs_rule(m, cost_type):
    ...
    if cost_type == "Invest":
        ...
        cost = sum(m.cap_pro[p] * m._data["pro_fixcost"][p] for p in m.pro_tuples)
        return m.costs[cost_type] == cost

    elif cost_type == "Variable":
        cost = sum(
            m.tau_pro[(tm,) + p] * m.weight * m._data["pro_varcost"][p]
            for tm in m.tm
            for p in m.pro_tuples
        )
        return m.costs[cost_type] == cost
        ...
        return m.costs[cost_type] == sum(
            m.e_co_stock[(tm,) + c] * m.weight * m._data["com_prices"][c]
            for tm in m.tm
